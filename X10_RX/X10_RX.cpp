...
{
	EICRA = 0b00000001; //Interrupt 0 aktiveres p� rising edge og falling edge
	EIMSK = 0b00000001; // Interrupt 0 enables
	DDRH &= ~(1<<5); // PORTH5 (pin 8) s�ttes til at v�re input 
...
bool X10_RX::dataReady(bool dataBit)
{
	...
	{
		// Forskyd startBuffer 1 til h�jre
		for (int i = 0; i < 7; i++)
		{
			startBuffer_[i] = startBuffer_[i + 1];
		}
		
		// Inds�t det databit som er modtaget p� den bagerste plads
